...
class SessionService:
    async def create_session(
        ...
        if title == "新对话":
            ...
            if existing_empty_session:
                logger.info(f"Found existing empty '新对话' session: {existing_empty_session['_id']}")
    ...
    async def get_sessions(
        ...
        try:
            ...
            cursor = db.sessions.find(query).sort("updated_at", -1).skip(skip).limit(limit)
    ...
    async def delete_session(self, session_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Found session to delete: {session_id}, user_id: {session.get('user_id')}")
