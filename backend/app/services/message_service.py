...
class MessageService:
    async def add_message(
        ...
        if db is None:
            logger.error(
                f"数据库连接不可用，无法保存消息: session_id={session_id}, role={role}"
            )
            return None
        ...
        try:
            ...
            if result.inserted_id:
                ...
                logger.info(
                    f"消息保存成功: id={result.inserted_id}, session={session_id}, role={role}"
                )
                return message_doc
            else:
    ...
    async def get_messages_paginated(
        ...
        try:
            ...
            fetch_limit = limit + 1
            cursor_obj = (
                db.messages.find(query).sort("_id", sort_direction).limit(fetch_limit)
            )
    ...
    async def search_messages(
        ...
        try:
            ...
            cursor = (
                db.messages.find(query).sort("created_at", -1).skip(skip).limit(limit)
            )
            return await cursor.to_list(length=limit)
        except Exception as e:
    ...
    async def get_messages_for_context(
        ...
        try:
            cursor = (
                db.messages.find({"session_id": ObjectId(session_id)})
                .sort("created_at", 1)
                .limit(limit)
            )
