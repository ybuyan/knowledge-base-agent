...
from app.mcp.auth_middleware import AuthContext, MCPAuthMiddleware
...
from app.mcp.transport import sse_generator
...
async def mcp_endpoint(
    request: Request, auth_context: Optional[AuthContext] = Depends(get_auth_context)
) -> JSONResponse:
    """
    ...
    from app.mcp.protocol import JSONRPCError, JSONRPCRequest, MCPErrorCode
...
async def list_public_resources() -> JSONResponse:
    ...
    return JSONResponse(
        content={"resources": [r.model_dump() for r in public_resources]}
    )
...
async def create_api_key(
    request: Request, auth_context: AuthContext = Depends(require_auth_context)
) -> JSONResponse:
    """
    创建 API Key（管理员端点）
...
async def revoke_api_key(
    key: str, auth_context: AuthContext = Depends(require_auth_context)
) -> JSONResponse:
    """
    撤销 API Key（管理员端点）
...
async def list_api_keys(
    ...
    return JSONResponse(content={"api_keys": [key.model_dump() for key in api_keys]})
...
async def get_audit_logs(
    ...
    return JSONResponse(content={"logs": [log.model_dump() for log in logs]})
