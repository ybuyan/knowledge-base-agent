...
class RateLimitExceeded(Exception):
    ...
    def __init__(self, user_id: str, retry_after: int):
        ...
        super().__init__(f"Rate limit exceeded for user {user_id}. Retry after {retry_after} seconds.")


class RateLimiter:
    """速率限制器（基于滑动窗口算法）"""
    
    def __init__(self, mongo_client: Optional[AsyncIOMotorClient] = None, use_redis: bool = False):
        """
        初始化速率限制器
    ...
    async def check_rate_limit(
        ...
        if count >= rate_limit:
            # 计算需要等待的时间
            retry_after = await self._calculate_retry_after(user_id, window_start, window_seconds)
    ...
    async def cleanup_expired_records(self, retention_hours: int = 24) -> int:
        """
        清理过期的速率限制记录
    ...
    async def check_ip_rate_limit(
        ...
        Args:
            ip_address: IP 地址
            rate_limit: 每个时间窗口的请求限制，None 使用默认值
            window_seconds: 时间窗口大小（秒）
        ...
        if rate_limit is None:
            rate_limit = settings.global_rate_limit_per_ip
    ...
    async def get_user_stats(self, user_id: str, hours: int = 1) -> dict:
        """
        获取用户的速率限制统计信息
        ...
        Returns:
            dict: 统计信息
                ...
                - requests_per_minute: 每分钟平均请求次数
                - peak_minute: 峰值分钟的请求次数
        """
        ...
        peak_minute = results[0]["count"] if results else 0
