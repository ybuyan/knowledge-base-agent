...
async def keyword_search(query: str, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    返回:
        [{"content": str, "metadata": dict, "keyword_score": float}, ...]
    """
    from app.core.mongodb import get_mongo_db

    db = get_mongo_db()
    if db is None:
        logger.warning("KeywordIndex: MongoDB 未连接，返回空结果")
        return []
    ...
    # 只查包含至少一个查询词的文档
    cursor = collection.find({"terms": {"$in": list(query_terms)}})

    scored: List[Dict[str, Any]] = []
    async for doc in cursor:
        doc_terms = set(doc.get("terms", []))
        overlap = query_terms & doc_terms
        score = len(overlap) / (len(query_terms) + 1e-9)
        scored.append({
            "content": doc["content"],
