...
class LLMGenerator(BaseProcessor):
    ...
    async def process(
        ...
        logger.info(
            "🤖 [LLM] 开始处理 | prompt_template='%s' | stream=%s",
            prompt_template_id,
            stream,
        )
        ...
        if not prompt_config:
            logger.error(
                "❌ [LLM] Prompt 模板未找到 | template_id='%s'", prompt_template_id
            )
            raise ValueError(f"Prompt template not found: {prompt_template_id}")

        logger.info(
            "📄 [LLM] 加载 Prompt 配置 | template_id='%s' | name='%s'",
            prompt_template_id,
            prompt_config.get("name", ""),
        )
        ...
        logger.debug(
            "   变量映射: %s",
            {
                k: f"{str(v)[:50]}..." if len(str(v)) > 50 else v
        ...
        logger.info(
            "💬 [LLM] 准备调用 LLM | system_prompt_length=%d | user_prompt_length=%d | history_length=%d",
            len(system_prompt),
            len(user_prompt),
            len(history),
        )
        ...
        logger.info(
            "✅ [LLM] LLM 调用完成 | answer_length=%d | prompt_template='%s'",
            len(answer),
            prompt_template_id,
        )
        ...
        if not answer or len(answer.strip()) == 0:
            logger.error(
                "⚠️  [LLM] 警告：LLM 返回空内容！| user_prompt='%s' | system_prompt_length=%d | history_length=%d",
                user_prompt[:100],
                len(system_prompt),
                len(history),
            )
